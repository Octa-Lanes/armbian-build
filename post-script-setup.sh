...
#   3  Inject all files from octimus-i-resources → userdata
# ============================================================
...
# ============================================================
#  PART 1 – Format Extra Partitions (recovery & userdata)
# ============================================================
...
echo "[✏️] Setting partition labels..."
...
echo "[🔍] Partition table after formatting:"
lsblk "$LOOP" -o NAME,LABEL,SIZE,TYPE

# ============================================================
#  PART 2 – Update fstab in RootFS
# ============================================================

if [ -b "${LOOP}p3" ]; then
    echo "[🧠] Mounting rootfs (${LOOP}p3)..."
    mkdir -p "$MNT_ROOT"
...
EOF
    ...
    echo "⚠️  Partition 3 (rootfs) not found! Skipped fstab update."
fi
...
# ============================================================
#  PART 3 – Inject Resources into /userdata
# ============================================================

if [ -b "${LOOP}p4" ]; then
    echo "[📦] Mounting userdata (${LOOP}p4)..."
    ...
    if (( FREE_SPACE < TOTAL_SIZE + 50 )); then
        echo "[⚠️] Warning: Only ${FREE_SPACE} MB free, source = ${TOTAL_SIZE} MB"
    fi

    echo "[🚚] Copying all files from ${SRC_DIR} → ${MNT_USERDATA}"
    cp -av "${SRC_DIR}/." "$MNT_USERDATA/"

    sync
    umount "$MNT_USERDATA"
    rmdir "$MNT_USERDATA"
    echo "[✅] Files successfully injected to /userdata"
else
    echo "⚠️  Partition 4 not found, skipping resource injection"
fi
...
echo "[🎉] All steps completed successfully for: $IMG_FILE"
